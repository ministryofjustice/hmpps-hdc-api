...
`smoke-dev.sh` is a bash script that runs a simple smoke test against the hmpps-hdc-api in the dev environment.
...
Dependencies: `jq`, `kubectl` (with access to the `licences-dev` namespace).
