...
class MigrationControllerTest : SqsIntegrationTestBase() {
  ...
  fun `When licence has only unknown versioned conditions then do not migrate licence to CVL`() {
    ...
    // Then
    response.expectStatus().isBadRequest
    assertThat(migrationRepository.getMigrationLog(1L, false, retry = false)).isEqualTo("Licence additional conditions version not determined!, status:INACTIVE OUT Ard:2025-04-17 Crd:2025-04-12 Led:2028-03-30 Hdcad:2025-04-30 PRRD:2025-04-21")
  }
  ...
  fun `Migrate licence to CVL migration fails because of a http 400 error the migration is retryable`() {
    ...
    assertThat(migrationRepository.getMigrationLog(1L, false, retry = false)).isEqualTo("it does not exist, status:INACTIVE OUT Ard:2025-04-17 Crd:2025-04-12 Led:2028-03-30 Hdcad:2025-04-30 PRRD:2025-04-21")
  }
  ...
  fun `Migrate licence to CVL migration fails because of a http Other error the migration is retryable`() {
    ...
    assertThat(migrationRepository.getMigrationLog(1L, false, retry = true)).isEqualTo("Service has failed, status:INACTIVE OUT Ard:2025-04-17 Crd:2025-04-12 Led:2028-03-30 Hdcad:2025-04-30 PRRD:2025-04-21")
  }
