...
BASE_URL=https://hdc-api-dev.hmpps.service.justice.gov.uk
...
get_token() {
  local SECRETS
  SECRETS="$(kubectl -n "${NAMESPACE}" get secret licences -o json)"
  local CLIENT_ID
  CLIENT_ID=$(echo "${SECRETS}" | jq -r ".data.ADMIN_API_CLIENT_ID | @base64d")
  local CLIENT_SECRET
  CLIENT_SECRET=$(echo "${SECRETS}" | jq -r ".data.ADMIN_API_CLIENT_SECRET | @base64d")
  local AUTH_BASIC
...
echo "=== hmpps-hdc-api smoke test (dev) ==="
echo
